...
Operational runtime can reference the managed artifact with either
`HEADROOM_OPENCODE_PLUGIN_PATH=/opt/headroom/opencode-plugin/package/dist/entry.opencode.js`
or `HEADROOM_OPENCODE_PLUGIN_ARTIFACT_DIR=/opt/headroom/opencode-plugin/package`.
If either variable is set but the entry file is missing, Headroom fails with an
explicit install/rebuild error instead of falling back to a development checkout.
