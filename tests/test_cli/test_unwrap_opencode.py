...
def test_wrap_unwrap_no_preexisting_config_leaves_no_headroom_wiring(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Wrap then unwrap with no pre-existing config leaves no Headroom wiring.

    Exercises the exact bug path: the MCP registrar creates ``opencode.json``
    before provider injection.  The old ``inject_opencode_provider_config()``
    would snapshot that newly-created file as the backup, contaminating it with
    Headroom MCP entries.  A later unwrap would restore the contaminated backup
    and leave Headroom wiring behind.
    """
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("HEADROOM_CONTEXT_TOOL", raising=False)
    set_test_home(monkeypatch, tmp_path)
    ...
    # Mock only the opencode binary check; other ``shutil.which`` calls
    # (especially ``resolve_headroom_command`` → ``shutil.which("headroom")``
    # inside the MCP registrar) must resolve normally so the MCP spec written
    # during wrap matches the expected spec used during unwrap.
    _real_which = wrap_mod.shutil.which
    ...
    # Wrap with MCP enabled so the registrar writes opencode.json.
    # Serena is disabled via flag to stay test-deterministic (no uvx).
    # Proxy and launch subprocesses are mocked by the isolate_proxy_startup
    # fixture and the patch stack below.
    with patch.object(wrap_mod.shutil, "which", side_effect=_which):
        with patch.object(wrap_mod, "_launch_tool", side_effect=SystemExit(0)):
            with patch.object(wrap_mod, "_ensure_rtk_binary", return_value=Path("/tmp/rtk")):
                runner.invoke(
                    main,
    ...
    assert config_file.exists(), "wrap must create opencode.json"
    assert not backup_file.exists(), (
        "Backup must not be created when no pre-existing config existed"
    )
    ...
    assert result.exit_code == 0, result.output
    # After unwrap with no pre-existing config, no Headroom wiring may remain.
    if config_file.exists():
        config = json.loads(config_file.read_text(encoding="utf-8"))
        assert "headroom" not in config.get("provider", {}), (
            "headroom provider left behind after unwrap"
        )
        assert "headroom" not in config.get("mcp", {}), "headroom MCP left behind after unwrap"
